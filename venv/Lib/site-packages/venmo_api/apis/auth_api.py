...
class AuthenticationApi(object):
    ...
    def login_with_credentials(self, username: str, password: str) -> str:
        ...
        if response.get('body').get('error'):
            # access_token = self.__two_factor_process_begin(response=response)
            otp_secret = self.__two_factor_process_begin(response=response)
    ...
    def __two_factor_process_begin(self, response: dict):
        otp_secret = response['headers'].get('venmo-otp-secret')
        print(f"otp_secret from venmo response={otp_secret}")
        if not otp_secret:
        ...
        return otp_secret
        # now time to ask user for otp_password
